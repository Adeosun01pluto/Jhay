...
const FAQItem = ({ question, answer }) => {
  ...
  return (
    <div className="border-b border-gray-200 py-4">
      <button
        className="flex justify-between items-center w-full text-left font-semibold text-lg text-gray-700 hover:text-[#FF900D] focus:outline-none"
        onClick={() => setIsOpen(!isOpen)}
      >
        <span>{question}</span>
        {isOpen ? <FaChevronUp className="text-[#FF900D]" /> : <FaChevronDown className="text-gray-400" />}
      </button>
  ...
  const ContactInfo = ({ icon, title, content }) => (
    <div className="flex items-start mb-6">
      ...
      <div>
        ...
        <p className="text-sm md:text-md text-gray-600 text-wrap">{content}</p>
      </div>
    </div>
  );
                ...
                <div>
                  ...
                  <form>
                    <div className="mb-4">
                      ...
                      <input type="text" id="name" name="name" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#FF900D]" required />
                    </div>
                    <div className="mb-4">
                      <label htmlFor="email" className="block text-gray-700 mb-2">Email</label>
                      <input type="email" id="email" name="email" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#FF900D]" required />
                    </div>
                    <div className="mb-4">
                      <label htmlFor="message" className="block text-gray-700 mb-2">Message</label>
                      <textarea id="message" name="message" rows="4" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#FF900D]" required></textarea>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {guides.map((guide, index) => (
              <a
                key={index}
                href={guide.link}
                className="bg-white rounded-lg shadow-md p-6 flex items-center space-x-4 hover:shadow-lg transition duration-300"
              >
                <FaBook className="text-3xl text-[#FF900D]" />
                <span className="text-lg font-medium text-gray-700">{guide.title}</span>
              </a>
            ))}
          </div>
        </section> */}
